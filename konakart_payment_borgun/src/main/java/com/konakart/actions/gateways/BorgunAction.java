...
/**
 * This class is an Action class for sending credit card details to Borgun and receiving
 * confirmation
 */
public class BorgunAction extends BaseGatewayAction {
    /**
     * The <code>Log</code> instance for this application.
     */
    protected Log log = LogFactory.getLog(BorgunAction.class);
    ...
    private static final String ORDER_HISTORY_COMMENT_OK = "Payment successful. Borgun Ref Number = ";

    private static final String ORDER_HISTORY_COMMENT_KO = "Payment not successful. Borgun Reply = ";
    ...
    public String execute() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("BorgunAction: code = " + code);
        }
        ...
        try {
            ...
            boolean borgunDebugMode = log.isDebugEnabled();

            // Check to see whether the user is logged in
            custId = this.loggedIn(request, response, kkAppEng, "Checkout");
            if (custId < 0) {
                if (log.isDebugEnabled()) {
                    log.debug("BorgunAction: NotLoggedIn");
                }
                return KKLOGIN;
            }
            ...
            if (redirForward != null) {
                if (log.isDebugEnabled()) {
                    log.debug("BorgunAction: Redirect SSL");
                }
            ...
            if (log.isDebugEnabled()) {
                log.debug("BorgunAction: Order " + order.getId() + " validated");
            }
            ...
            if (log.isDebugEnabled()) {
                log.debug("BorgunAction: Post the payment details to the gateway");
            }
            ...
            if (borgunDebugMode) {
                ...
                try {
                    String outputFilename = getLogFileDirectory(kkAppEng) + "borgun_resp_"
                            + order.getId() + ".txt";
            ...
            StringTokenizer st1 = new StringTokenizer(gatewayResp.toString(), new Character((char) 0x1C).toString());
            ...
            try {
                ...
                if (log.isWarnEnabled()) {
                    log.warn("Exception communicating with Borgun");
                    e.printStackTrace();
                }
